...
def txt2image(exec_path, prompt, samples, steps, seed, width, height, out_dir, ckpt_model):
	if (ckpt_model != ""):
		command = f"conda run -n ldm python '{exec_path}' --prompt '{prompt}' --n_samples {samples} --n_iter 1 --ddim_steps {steps} --seed {seed} --H {height} --W {width} --outdir {out_dir} --skip_grid --ckpt {ckpt_model}"
	else:
		command = f"conda run -n ldm python '{exec_path}' --prompt '{prompt}' --n_samples {samples} --n_iter 1 --ddim_steps {steps} --seed {seed} --H {height} --W {width} --outdir {out_dir} --skip_grid"
