...
class NoiseScheduleVP:
    def __init__(
        ...
        ***
        ...
        if schedule == 'discrete':
            ...
            self.cosine_t_max = math.atan(self.cosine_beta_max * (
                1. + self.cosine_s) / math.pi) * 2. * (1. + self.cosine_s) / math.pi - self.cosine_s
    ...
    def inverse_lambda(self, lamb):
        ...
        if self.schedule == 'linear':
            ...
            t = interpolate_fn(log_alpha.reshape((-1, 1)), torch.flip(self.log_alpha_array.to(
                lamb.device), [1]), torch.flip(self.t_array.to(lamb.device), [1]))
            return t.reshape((-1,))
        else:
            log_alpha = -0.5 * \
                torch.logaddexp(-2. * lamb, torch.zeros((1,)).to(lamb))

            def t_fn(log_alpha_t): return torch.arccos(torch.exp(
                log_alpha_t + self.cosine_log_alpha_0)) * 2. * (1. + self.cosine_s) / math.pi - self.cosine_s
            t = t_fn(log_alpha)
            return t
...
class DPM_Solver:
    ...
    def data_prediction_fn(self, x, t):
        ...
        if self.thresholding:
            ...
            s = expand_dims(torch.maximum(
                s, torch.ones_like(s).to(s.device)), dims)
        ...
        if self.predict_x0:
            ...
            if solver_type == 'dpm_solver':
                ...
                x_t = (
                    expand_dims(sigma_t / sigma_s, dims) * x
                    - expand_dims(alpha_t * phi_1, dims) * model_s
                    + (1. / r1) * expand_dims(alpha_t *
                                              ((torch.exp(-h) - 1.) / h + 1.), dims) * (model_s1 - model_s)
                )
        else:
            phi_11 = torch.expm1(r1 * h)
            phi_1 = torch.expm1(h)
    ...
    def singlestep_dpm_solver_third_update(self, x, s, t, r1=1./3., r2=2./3., model_s=None, model_s1=None, return_intermediate=False, solver_type='dpm_solver'):
        ...
        if self.predict_x0:
            ...
            x_s2 = (
                ...
                - r2 / r1 * expand_dims(sigma_s2 * phi_22,
                                        dims) * (model_s1 - model_s)
            )
            model_s2 = self.model_fn(x_s2, s2)
            if solver_type == 'dpm_solver':
    ...
    def multistep_dpm_solver_third_update(self, x, model_prev_list, t_prev_list, t, solver_type='dpm_solver'):
        ...
        lambda_prev_2, lambda_prev_1, lambda_prev_0, lambda_t = ns.marginal_lambda(
            t_prev_2), ns.marginal_lambda(t_prev_1), ns.marginal_lambda(t_prev_0), ns.marginal_lambda(t)
        log_alpha_prev_0, log_alpha_t = ns.marginal_log_mean_coeff(
            t_prev_0), ns.marginal_log_mean_coeff(t)
        sigma_prev_0, sigma_t = ns.marginal_std(t_prev_0), ns.marginal_std(t)
        alpha_t = torch.exp(log_alpha_t)
    ...
    def dpm_solver_adaptive(self, x, order, t_T, t_0, h_init=0.05, atol=0.0078, rtol=0.05, theta=0.9, t_err=1e-5, solver_type='dpm_solver'):
        ...
        if order == 2:
            ...
            def lower_update(x, s, t): return self.singlestep_dpm_solver_second_update(
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=3, skip_type='time_uniform',
        ...
        if method == 'adaptive':
            ...
            if method == 'singlestep':
                orders = self.get_orders_for_singlestep_solver(
                ...
                orders = [order, ] * K
                timesteps = self.get_time_steps(
                    skip_type=skip_type, t_T=t_T, t_0=t_0, N=(K * order), device=device)
            with torch.no_grad():
                i = 0
                for order in orders:
                    vec_s, vec_t = timesteps[i].expand(
                        x.shape[0]), timesteps[i + order].expand(x.shape[0])
                    h = self.noise_schedule.marginal_lambda(
                        timesteps[i + order]) - self.noise_schedule.marginal_lambda(timesteps[i])
                    r1 = None if order <= 1 else (self.noise_schedule.marginal_lambda(
                        timesteps[i + 1]) - self.noise_schedule.marginal_lambda(timesteps[i])) / h
                    r2 = None if order <= 2 else (self.noise_schedule.marginal_lambda(
                        timesteps[i + 2]) - self.noise_schedule.marginal_lambda(timesteps[i])) / h
