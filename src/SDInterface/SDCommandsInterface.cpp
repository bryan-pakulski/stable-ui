...
void SDCommandsInterface::textToImage(std::string sdModelPath, std::string prompt, std::string negative_prompt,
                                      int samples, int steps, double cfg, int seed, int width, int height,
                                      int &renderState) {
  std::string functionName = "txt2img";
  std::string execPath = CONFIG::SD_SERVER_CLIENT.get();
  std::string outDir = CONFIG::OUTPUT_DIRECTORY.get();

  arguments->emplace_back(std::unique_ptr<base_type>(new d_type<std::string>('s', "sd_model_path", sdModelPath, 0)));
  arguments->emplace_back(std::unique_ptr<base_type>(new d_type<std::string>('s', "exec_path", execPath, 1)));
  ...
  arguments->emplace_back(std::unique_ptr<base_type>(new d_type<double>('f', "scale", cfg, 6)));
  arguments->emplace_back(std::unique_ptr<base_type>(new d_type<int>('d', "seed", seed, 7)));
  arguments->emplace_back(std::unique_ptr<base_type>(new d_type<int>('d', "width", width, 8)));
  arguments->emplace_back(std::unique_ptr<base_type>(new d_type<int>('d', "height", height, 9)));
  arguments->emplace_back(std::unique_ptr<base_type>(new d_type<std::string>('s', "out_dir", outDir, 10)));
  arguments->emplace_back(std::unique_ptr<base_type>(new d_type<int>('d', "n_iter", 1, 11)));
...
void SDCommandsInterface::imageToImage(std::string path, std::string prompt, std::string negative_prompt, int samples,
  ...
  arguments->emplace_back(std::unique_ptr<base_type>(new d_type<int>('d', "seed", seed, 7)));
  arguments->emplace_back(std::unique_ptr<base_type>(new d_type<std::string>('s', "out_dir", outDir, 8)));
