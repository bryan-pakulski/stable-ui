// All credit for this file browser goes to AirGuanZ
// https://github.com/AirGuanZ/imgui-filebrowser/tree/master
#pragma once
...
inline ImGui::FileBrowser::FileBrowser(ImGuiFileBrowserFlags flags)
    : width_(700), height_(450), posX_(0), posY_(0), flags_(flags), openFlag_(false), closeFlag_(false),
      isOpened_(false), ok_(false), posIsSet_(false),
...
inline ImGui::FileBrowser::FileBrowser(const FileBrowser &copyFrom) : FileBrowser() { *this = copyFrom; }
    ...
    reserveHeight += GetFrameHeightWithSpacing();
  {
    ...
    for (auto &rsc : fileRecords_) {
      ...
      if (Selectable(rsc.showName.c_str(), selected, ImGuiSelectableFlags_DontClosePopups)) {
        const bool multiSelect = (flags_ & ImGuiFileBrowserFlags_MultipleSelection) &&
...
inline const class std::filesystem::path &ImGui::FileBrowser::GetPwd() const noexcept { return pwd_; }
...
#if defined(__cpp_lib_char8_t)
inline std::string ImGui::FileBrowser::u8StrToStr(std::u8string s) { return std::string(s.begin(), s.end()); }
#endif
