...
public:
  ...
  void contentBrowser() {
    if (ImGui::CollapsingHeader("Content Browser")) {
      ImGui::SetNextWindowSizeConstraints(ImVec2(CONFIG::IMGUI_TOOLS_WINDOW_WIDTH.get(), 380.0f),
                                          ImVec2(CONFIG::IMGUI_TOOLS_WINDOW_WIDTH.get(), 380.0f));
  ...
  void previewWindow() {
    if (ImGui::CollapsingHeader("Browser Preview")) {
      if (!m_selected_file.empty() && m_preview_image->textured) {
  ...
  void maskPreviewWindow() {
    if (ImGui::CollapsingHeader("Mask Preview")) {
      if (m_image_mask) {
